...
const MainInfo: React.FC<MainInfoProps> = ({ product, userId }) => {
  ...
  const handleAddToCart = async () => {
    ...
    if (!isColorOk || !isStorageOk) {
      ...
      if (hasColorOptions && !selectedColor) missingOptions.push("color");
      if (hasStorageOptions && !selectedMemory) missingOptions.push("storage");
      ...
      <div className="flex flex-col lg:flex-row">
        <div className="flex-col w-full h-auto lg:w-1/2">
          <div className="lg:flex lg:justify-center lg:items-center">
            ...
            <div className="flex justify-center items-center flex-grow">
              <img src={mainImageToShow} alt={product.name} className="w-auto h-auto max-h-[329px] lg:max-h-[516px] object-contain" />
            </div>
          </div>
        ...
        <div className="lg:w-1/2 mt-8 lg:mt-0 flex justify-center">
          <div className="w-full h-auto lg:w-[90%]">
            <h1 className="font-bold text-[40px] leading-[40px]">{product.name}</h1>
            ...
            <h2 className="mt-5 text-sm text-zinc-600">{product.description}</h2>
